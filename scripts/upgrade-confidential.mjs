// Upgrade the veilex_contracts package in place (adds the Nitro-attestation path
...
//   sui move build --dump-bytecode-as-base64 > /tmp/veilex_bytecode.json
//   SUI_PRIVATE_KEY=$(sui keytool export --key-identity veilex-deployer --json | grep -oE 'suiprivkey1[a-z0-9]+') node scripts/upgrade-confidential.mjs
...
const { modules, dependencies, digest } = JSON.parse(readFileSync("/tmp/veilex_bytecode.json", "utf8"));
